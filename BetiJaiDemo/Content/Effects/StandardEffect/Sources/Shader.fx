...
VSOutputPbr VertexFunction(VSInputPbr input)
{
	...
	float3 bitangent = cross(input.Tangent.xyz, input.Normal) * input.Tangent.w;
	output.TangentWS = normalize(mul(float4(tangent, 0), World).xyz);
	output.BitangentWS = normalize(mul(float4(bitangent, 0), World).xyz);
#endif
#endif
